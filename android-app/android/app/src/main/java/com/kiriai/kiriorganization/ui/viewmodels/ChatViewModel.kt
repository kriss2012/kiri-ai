...
class ChatViewModel @Inject constructor(
    ...
    init {
        ...
        observeUserData()
        observeConnectivity()
        // Removed direct loadConversations() here to avoid race condition with AuthInterceptor
    }
    ...
    fun sendMessage() {
        ...
        // Important: We keep the IMAGE_URI tag in the content so the UI knows to render it.
        ...
        viewModelScope.launch {
            try {
                ...
                // If there's a file, we MUST use the upload endpoint.
    ...
    private suspend fun uploadFileAndSend(message: String, uri: Uri): Result<ChatResponse> {
        return try {
            val file = if (uri.scheme == "file") {
                ...
                // Should not happen as we copy to internal storage
                val contentResolver = getApplication<Application>().contentResolver
            ...
            val filePart = MultipartBody.Part.createFormData("file", _uiState.value.selectedFileName ?: file.name, requestFile)
            
            // If the message is empty, the server might reject it or not know what to do.
            ...
            val currentId = _uiState.value.currentConversationId
            val convIdPart = if (currentId != null && currentId.isNotBlank()) {
                MultipartBody.Part.createFormData("conversationId", currentId)
            } else null

            android.util.Log.d("Kiri_DEBUG", "ChatViewModel: uploadFileAndSend - prompt='$prompt', convId=$currentId file_size=${file.length()}")
            chatRepository.sendMessageWithFile(contentPart, modelPart, convIdPart, filePart)
        } catch (e: Exception) {
            android.util.Log.e("Kiri_DEBUG", "ChatViewModel: uploadFileAndSend exception", e)
            Result.failure(e)
        }
    }
