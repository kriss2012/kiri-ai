...
class ChatRepository @Inject constructor(
    ...
    suspend fun sendMessageWithFile(
        content: okhttp3.MultipartBody.Part,
        model: okhttp3.MultipartBody.Part,
        conversationId: okhttp3.MultipartBody.Part?,
        filePart: okhttp3.MultipartBody.Part
    ): Result<ChatResponse> {
        return try {
            val response = chatApi.sendMessageWithFile(content, model, conversationId, filePart)
            handleResponse(response)
        } catch (e: java.net.UnknownHostException) {
    ...
    suspend fun deleteAllConversations(): Result<GenericResponse> {
        return try {
            val response = chatApi.deleteAllConversations()
            val body = response.body()
            if (response.isSuccessful && body != null) {
                Result.success(body)
            } else {
                ...
                Result.failure(Exception(message ?: "Clear history failed (${response.code()})"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
